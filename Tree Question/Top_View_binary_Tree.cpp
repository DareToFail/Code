...
void printverticalorder(vector<pair<int,int>> &vertical,vector<int> &level){
	int n = level.size();
	int d = INT_MAX;
...
void printtopview(vector<int> level,vector<pair<int,int>> vertical){
     int n = level.size();
     int d = vertical[0].first;
     cout<<level[vertical[0].second]<<" ";
     for(int i=1;i<n;i++){
        if(d==vertical[i].first)
            continue;
        cout<<level[vertical[i].second]<<" ";
...
void printbottomview(vector<int> level,vector<pair<int,int>> vertical){
     int n = level.size();
     int d = INT_MAX;
     for(int i=0;i<n;){
        while(d==vertical[i+1].first){
        	i++;
		}
        cout<<level[vertical[i].second]<<" ";
        d = vertical[i+1].first;
        i++;
     }
 }
int main(){
	...
	cout<<"\nInorder of constructed tree is as follow :- ";
	inorder(root);
	vector<int> level;
	vector<int> hd;
	...
	printverticalorder(vertical,level);
	//Top view
	cout<<"\n\nTop View of a Binary Tree using Level order :- ";
	printtopview(level,vertical);
	cout<<"\n\nBottom View of a Binary Tree using Level order :- ";
	printbottomview(level,vertical);
	return 0;
}
